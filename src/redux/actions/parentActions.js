...
export const accpetChildRequest = (email, parentId) => async (dispatch) => {
  try {
    document.body.classList.add("loading-indicator");
    const result = await API.acceptChildRequest(email, parentId);

    dispatch({
      type: ACCEPT_REJECT_PARENT_REQUEST_SUCCESS,
    });
    dispatchSuccess(
      dispatch,
      "Request Accepted",
    ...
    document.body.classList.remove("loading-indicator");
    dispatchError(dispatch, err, ACCEPT_REJECT_PARENT_REQUEST_FAILURE);
  }
};
