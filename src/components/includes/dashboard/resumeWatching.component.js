...
const Box = (props) => {
  ...
  const linkToVideotLesson = `/content/${slugify(
    props.item.courseId.name
  )}/${slugify(props.item.subjectId.mainSubjectId.name)}/${slugify(
    props.item.lessonId?.title
  )}/${props.item.lessonId?.videoUrls[0]._id}?courseId=${
    props.item.courseId.id
  }&subjectId=${props.item.subjectId.id}&lessonId=${
    props.item.lessonId?.id
  }&videoId=${props.item.lessonId?.videoUrls[0]._id}&termId=${
    props.item.termId
  }`;
  return (
    <div className="col-md-2 topTen">
      <Link to={linkToVideotLesson}>
        ...
        <p>
          {props.item && props.item.lessonId
            ? props.item.lessonId.title.length > 40
              ? props.item.lessonId.title.substr(0, 37) + "..."
              : props.item.lessonId.title
            : "unknown"}
        </p>
