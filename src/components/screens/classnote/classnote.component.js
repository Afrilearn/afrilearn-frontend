...
const ClassNote = (props) => {
  ...
  const Syllabus = () => {
    return (
      <div className="navigate-terms">
        <div className="border-bottom p-2 px-3">
          <h3 className="baloo">Subject Syllabus</h3>
        </div>
        <div class="accordion" id="accordionExample">
          {terms.map((i, index) => (
            <div class="accordion-item" key={index}>
              <h2 class="accordion-header" id={`headingmobile${index}${i._id}`}>
                <button
                  class="accordion-button nunito"
                  type="button"
                  data-bs-toggle="collapse"
                  data-bs-target={`#collapsemobile${index}${i._id}`}
                  aria-expanded="true"
                  aria-controls={`collapsemobile${index}${i._id}`}
                >
                  <strong>{i.name}</strong>
                </button>
              </h2>
              <div
                id={`collapsemobile${index}${i._id}`}
                class="accordion-collapse collapse"
                aria-labelledby={`headingmobile${index}${i._id}`}
                data-bs-parent="#accordionExample"
              >
                <div class="accordion-body">
                  {i?.lessons?.map((lessonItem, lessonIndex) => (
                    <p
                      // data-bs-dismiss="modal"
                      key={lessonIndex}
                      className={`${
                        targetLesson._id === lessonItem._id && "green"
                      } nunito`}
                    >
                      <Link
                        style={{
                          pointerEvents: activeCoursePaidStatus
                            ? "all"
                            : "none",
                        }}
                        to={`/classnote/${
                        ...
                        }/${lessonItem && slugify(lessonItem.title)}?courseId=${
                          parsed.courseId
                        }&subjectId=${parsed.subjectId}&lessonId=${
                          lessonItem && lessonItem._id
                        }&termId=${lessonItem && lessonItem.termId}`}
                      >
                        {lessonItem.title}{" "}
                        {/* {!activeCoursePaidStatus && '()'} */}
                      </Link>
                      {!activeCoursePaidStatus && (
                        <a href="/select-pay">
                          <span className="badge bg-danger">Subscribe</span>
                        </a>
                      )}
                    </p>
                  ))}
  ...
  return (
    <span>
      <Helmet>
        <meta charSet="utf-8" />
        <title>
          {targetLesson ? targetLesson.title : "Classnote"} | Myafrilearn.com
        </title>
        <meta
          name="description"
          content={targetLesson ? targetLesson.content : "Classnote"}
        />
      </Helmet>
      ...
      <div
        class="modal fade classNoteModalSection"
        id="exampleModal"
        tabindex="-1"
        ...
        <div id="classNoteSecondSection" className="container-fluid relative">
                  ...
                  targetLesson.videoUrls.length > 0 && (
                    <li>
                      <DTooltip
                        placement="top"
                        trigger={["hover"]}
                        overlay={<span>Video Lesson</span>}
                      >
                    ...
                    >
                      <Speech
                        content={decodeEntities(
                          targetLesson && targetLesson.content
                        )}
                      />
                    </Link>
                  </DTooltip>
          ...
          <div className="row">
            ...
            <div className="col-md-12 stat">
              ...
              {targetLesson &&
                numberWithCommas(targetLesson.views) + " view(s)"}
            </div>
